"""
experiments/evaluate_test_dp.py — Final test-set evaluation for Paper 2 DP models.
...
Checkpoints follow Paper 2 naming convention:
  uniform    → checkpoints/best_DP-uniform-eps{label}-seed{N}.pt
  stratified → checkpoints/best_DP-stratified-eps5-seed{N}.pt

(ε_level, mode) combinations are read from results/privacy_utility.csv.
...
def main() -> None:
    ...
    if pcsv.exists():
        ...
        print(f"[evaluate_test_dp] privacy_utility.csv not found; using {len(combos)} defaults")
