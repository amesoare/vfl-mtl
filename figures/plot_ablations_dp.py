"""
figures/plot_ablations_dp.py - Paper 2 DP ablation figures.

Reads results/dp_ablations.csv and generates one figure per ablation:

  Abl 1 - dumbbell chart: uniform vs. stratified sigma at epsilon=5 (3 tasks, single panel)
  Abl 2 - dumbbell chart: gradient coupling rho and IHM inference AUC
           for related (IHM+Decomp) vs. unrelated (IHM+Pheno) task pairs (single panel)
  Abl 3 - line plot: per-task AUC vs. epsilon for embed_dim in {32, 64, 128}

Usage
-----
  python figures/plot_ablations_dp.py                          # unified figure
  python figures/plot_ablations_dp.py --abl 3                  # abl 3 only
  python figures/plot_ablations_dp.py --input results/dp_ablations.csv
"""
...
def main() -> None:
    ...
    if args.abl is None:
        plot_unified(df, df3, out / "ablations_dp.png")
    else:
        if args.abl == 1: plot_abl1(df, out / "ablations_dp_abl1.png")
        if args.abl == 2: plot_abl2(df, out / "ablations_dp_abl2.png")
        if args.abl == 3: plot_abl3(df3, out / "ablations_dp_abl3.png")
